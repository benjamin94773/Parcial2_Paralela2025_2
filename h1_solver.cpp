...
struct CmpH1{
      bool operator()(const NodeH1 &a, const NodeH1 &b) const{
            return (a.g + a.h) > (b.g + b.h);
      }
};

static inline int heuristicMisplaced(const string &s){
      int mis = 0;
      for (int i = 0; i < 16; ++i){
            if (s[i] != '#' && s[i] != TARGET[i]) ++mis;
      }
      return mis;
}
...
int aStarSearch(string start){
      ...
      priority_queue<NodeH1, vector<NodeH1>, CmpH1> pq;
      unordered_map<string,int> gScore;
      gScore.reserve(100000);
      gScore[start] = 0;
      pq.push({start, 0, heuristicMisplaced(start), startBlank});

      const int dRow[] = {-1, 1, 0, 0};
      const int dCol[] = {0, 0, -1, 1};

      while (!pq.empty()){
            NodeH1 cur = pq.top(); pq.pop();
