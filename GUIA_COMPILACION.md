...
Guarda como `test_all.sh`, ejecuta con:
```bash
chmod +x test_all.sh
./test_all.sh
```
...
Asegúrate de compilar con `-fopenmp`:
```bash
g++ -std=c++17 -O2 -fopenmp archivo.cpp -o ejecutable
```
...
Agrega flag `-pthread`:
```bash
g++ -std=c++17 -O2 -fopenmp -pthread archivo.cpp -o ejecutable
```
...
```bash
# Listar ejecutables
ls -lh board_printer board_moves board_available bsp_solver h1_solver h2_solver nxn_solver parallel_data tarea9_dominio tarea10_comparacion

# Verificar flags de OpenMP
ldd parallel_data | grep gomp
```
...
```bash
rm -f board_printer board_moves board_available bsp_solver h1_solver h2_solver nxn_solver parallel_data tarea9_dominio tarea9_metricas tarea10_comparacion
```
